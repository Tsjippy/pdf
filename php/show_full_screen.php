...
?>
    ...
    <div data-id="<?php echo esc_attr($id); ?>" class='full-screen-pdf-wrapper <?php echo esc_attr($class); ?>' style='z-index: 9999999;position: absolute;top: 0;left: 0;'>
        <div style='position: absolute; top: 0; left: 0; z-index: 99991; width:100vw; height:-webkit-fill-available; min-height:100vh; background-color: white;margin-top: -33px;'>
            <button type='button' id='close-full-screen' class='button small' style='position: sticky; z-index: 99992; <?php echo esc_attr($style); ?>' onclick='this.closest(".full-screen-pdf-wrapper").classList.add("hidden");'>
                <?php echo esc_attr($close); ?>
            </button>
            ...
            <iframe loading="lazy" src='<?php echo esc_url($url); ?>' style='<?php echo esc_attr($objectStyle); ?>' type='application/pdf' onload="this.closest('.full-screen-pdf-wrapper').querySelector('.loader-wrapper').classList.add('hidden')"></iframe>
        </div>
    </div>
<?php
